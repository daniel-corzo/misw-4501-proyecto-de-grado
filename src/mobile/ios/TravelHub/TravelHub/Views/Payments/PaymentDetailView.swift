...
struct PaymentDetailView: View {
    ...
    var hotel: Hotel
    var nights: Int
    /// When set, invoked after payment succeeds instead of showing only the payment toast.
    ...
    @State private var expirationDateError: AttributedString = ""

    private var total: Double {
        subtotal + taxesAndFees
    }

    private var isExternallyBlocked: Bool {
        supplementalBlocking?.wrappedValue == true
    }
    ...
    private var isPaymentFormValid: Bool {
        ...
        if self.expirationDate.isEmpty || !self.viewModel.expirationDateIsValid(self.expirationDate) {
            return false
        }
    ...
    var body: some View {
        VStack {
            ...
            VStack(spacing: 16) {
                ...
                .onChange(of: self.cardNumber) { _, newNumber in
                    self.cardNumber = formatCreditCardNumber(newNumber)
                }
                ...
                HStack(alignment: .top) {
                    FormTextFieldView(
                        ...
                        smallText: self.expirationDateError,
                        text: self.$expirationDate
                    )
                    .keyboardType(.numberPad)
                    .onChange(of: self.expirationDate) { _, newDate in
                        let formattedDate = formatExpirationDate(newDate)
                        self.expirationDate = formattedDate
                        
                        if !self.viewModel.expirationDateIsValid(formattedDate) {
                            self.expirationDateError = .init(String(localized: .Payment.expirationDateInvalid))
                            self.expirationDateError.foregroundColor = .red
                        } else {
                    ...
                    .onChange(of: self.cvv, { _, newValue in
                        self.cvv = formatCvv(newValue)
                    })
                    .keyboardType(.numberPad)
                }
            }  //: VStack Form
                ...
                Task {
                    ...
                    guard
                        let payment = await self.viewModel.pay(
                            ...
                            showSuccessToast: showPaymentToast
                        )
                    else { return }
                    if let onPaymentSuccess {
